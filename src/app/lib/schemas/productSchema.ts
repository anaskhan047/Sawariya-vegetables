...
const toNumber = (int = false, opts?: { nonnegative?: boolean; positive?: boolean }) =>
  z
    .preprocess((v) => (typeof v === "string" && v !== "" ? Number(v) : v), int ? z.number().int() : z.number())
    .refine((val) => {
...
// Accept either `Tag` (capital) or `tags` and normalize later in code
export const productSchema = z
  .object({
    ...
    // allow both, but make optional; we'll normalize in handler
